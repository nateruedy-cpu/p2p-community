...
const CHECKLIST_ITEMS = [
  ...
  { id: 'rest_day',         label: 'Today is a rest day',                     type: 'boolean', category: 'Morning Physical', skipIfChecked: 'gym' },
...
async function enterApp(user) {
  ...
  document.getElementById('nav-checkins-admin').classList.toggle('hidden', !(currentProfile && currentProfile.is_admin));
  ...
  if (currentProfile && currentProfile.is_admin) loadCheckinsAdminList();
  loadNotifications();
  subscribeRealtime();
  checkIntroStatus();
}
...
function renderSidenavAvatar() {
  setAvatarInto('sidenav-avatar', currentProfile);
}
...
// Sets an existing .avatar-circle container's content AND clears its background when a
// real photo is present — otherwise the panel-2 fill shows as a gray shape behind/around it.
function setAvatarInto(elId, profile) {
  const el = document.getElementById(elId);
  if (!el) return;
  el.innerHTML = avatarInnerHtml(profile);
  el.style.background = (profile && profile.avatar_url) ? 'transparent' : '';
}
...
function switchView(view) {
  ...
  if (view === 'checkins-admin') loadCheckinsAdminList();
}
...
const channelCache = {}; // channel -> { posts, reactions, comments } — lets search filter client-side without refetching
...
async function loadChannel(channel) {
  ...
  const { data: posts, error } = await supabaseClient
    .from('posts')
    .select('id, title, content, pinned, image_url, created_at, user_id, profiles(name, avatar_url)')
    .eq('channel', channel)
  ...
  channelCache[channel] = { posts: posts || [], reactions: [], comments: [] };
  ...
  if (posts && posts.length) {
    ...
    channelCache[channel].reactions = reactions || [];
    channelCache[channel].comments = comments || [];
  }
...
function applyChannelFilter(channel) {
  const cache = channelCache[channel];
  const feedEl = document.getElementById(`feed-${channel}`);
  if (!feedEl || !cache) return;

  const searchEl = document.getElementById(`search-${channel}`);
  ...
  if (!posts.length) {
    feedEl.innerHTML = query
      ? `<div class="empty-state">No posts match "${escapeHtml(rawQuery)}".</div>`
      : `<div class="empty-state">Nothing here yet.<br>Be the first to post.</div>`;
    return;
  }

  const postIds = new Set(posts.map(p => p.id));
  const reactions = cache.reactions.filter(r => postIds.has(r.post_id));
  const comments = cache.comments.filter(c => postIds.has(c.post_id));
...
function renderChannel(channel, posts, reactions, comments) {
  ...
  feedEl.innerHTML = posts.map((post, idx) => {
    ...
    const isMine = post.user_id === currentUser.id || (currentProfile && currentProfile.is_admin);
      ...
      <div class="comments-block">
        ${postComments.map(c => `
          <div class="comment-row">
            ...
            ${(c.user_id === currentUser.id || (currentProfile && currentProfile.is_admin)) ? `<button class="comment-delete" data-comment-delete="${c.id}" title="Delete comment">✕</button>` : ''}
          </div>
    ...
    return `
      <div class="post-card ${post.pinned ? 'pinned' : ''}">
        ${post.pinned ? '<div class="pinned-flag">📌 Pinned</div>' : ''}
        <div class="post-meta">
          ...
          <div class="post-meta-right">
            ...
            ${currentProfile && currentProfile.is_admin ? `<button class="post-pin" data-post-pin="${post.id}" data-pin-state="${post.pinned ? 'unpin' : 'pin'}" title="${post.pinned ? 'Unpin' : 'Pin to top'}">📌</button>` : ''}
  ...
  feedEl.querySelectorAll('[data-post-pin]').forEach(btn => {
    btn.addEventListener('click', async () => {
      const pin = btn.dataset.pinState === 'pin';
      const { error } = await supabaseClient.from('posts').update({ pinned: pin }).eq('id', Number(btn.dataset.postPin));
      if (error) { alert('Could not update pin: ' + error.message); return; }
      loadChannel(channel);
    });
  });
    ...
    return `
      <div class="chat-msg">
        ...
        <div class="chat-msg-footer">
          ...
          ${(msg.user_id === currentUser.id || (currentProfile && currentProfile.is_admin)) ? `<button class="chat-msg-reply" data-delete-msg="${msg.id}">Delete</button>` : ''}
        </div>
      </div>
    `;
  }).join('');
...
function initProfileModal() {
  ...
  openBtn.addEventListener('click', () => {
    ...
    setAvatarInto('modal-avatar-preview', currentProfile);
...
async function loadDmConversations() {
  ...
  lastMessageTimeCache = {};
  (myMessages || []).forEach(m => {
    const partnerId = m.sender_id === currentUser.id ? m.recipient_id : m.sender_id;
    if (!lastMessageTimeCache[partnerId]) lastMessageTimeCache[partnerId] = m.created_at; // first hit is newest, since ordered desc
  });
...
function renderMemberList(filterText) {
  ...
  // Recent conversations first (newest message at top), then everyone else alphabetically
  const sorted = [...filtered].sort((a, b) => {
    const at = lastMessageTimeCache[a.id];
    const bt = lastMessageTimeCache[b.id];
  ...
  listEl.innerHTML = sorted.map(m => `
    <div class="dm-convo-item ${activeDmPartnerId === m.id ? 'active' : ''}" data-partner="${m.id}">
      <button class="dm-convo-avatar-btn" data-view-profile="${m.id}" title="View profile">${avatarHtml(m, 'small')}</button>
      <span class="dm-convo-name">${escapeHtml(m.name)}</span>
      ${lastMessageTimeCache[m.id] ? `<span class="dm-convo-time">${formatTime(lastMessageTimeCache[m.id])}</span>` : ''}
...
async function openDmThread(partnerId) {
  ...
  document.getElementById('dm-thread-header').innerHTML = `
    <button class="dm-thread-header-btn" data-view-profile="${partnerId}">
      ${avatarHtml(partner, 'small')} <span>${escapeHtml(partner ? partner.name : 'Member')}</span>
    </button>
  `;
...
function renderDmThread() {
  ...
  const partnerProfile = allMembers.find(m => m.id === activeDmPartnerId);

  box.innerHTML = dmThreadCache.map(msg => {
    const mine = msg.sender_id === currentUser.id;
    const authorProfile = mine ? currentProfile : partnerProfile;
    return `
      <div class="dm-bubble-row ${mine ? 'mine' : ''}">
        ${!mine ? avatarHtml(authorProfile, 'small') : ''}
        <div class="dm-bubble-col">
          ...
          <div class="dm-bubble-footer">
            <span class="dm-bubble-time">${formatTime(msg.created_at)}</span>
            ${mine ? `<button class="dm-bubble-delete" data-dm-delete="${msg.id}">Delete</button>` : ''}
          </div>
        </div>
        ${mine ? avatarHtml(authorProfile, 'small') : ''}
      </div>
  ...
  box.querySelectorAll('[data-dm-delete]').forEach(btn => {
    btn.addEventListener('click', async () => {
      const { error } = await supabaseClient.from('direct_messages').delete().eq('id', Number(btn.dataset.dmDelete));
      if (error) { alert('Could not delete: ' + error.message); return; }
      loadDmThread();
    });
  });
...
async function loadLeaderboard() {
  ...
  const [{ data: members }, { data: allCheckins }] = await Promise.all([
    supabaseClient.from('profiles').select('id, name, avatar_url').eq('banned', false),
...
async function loadAdminPanel() {
  ...
  const [{ data: members }, { data: intros }] = await Promise.all([
    supabaseClient.from('profiles').select('id, name, avatar_url, can_announce, is_admin, banned, has_clients').order('name', { ascending: true }),
  ...
  listEl.innerHTML = (members || []).map(m => `
    <div class="admin-row ${m.banned ? 'banned' : ''}">
      ${avatarHtml(m, 'small')}
      <span class="admin-name">${escapeHtml(m.name)}${m.id === currentUser.id ? ' (you)' : ''}${m.banned ? ' <span class="admin-removed-badge">Removed</span>' : ''}</span>
      ...
      ${m.id !== currentUser.id ? `<button class="admin-remove-btn" data-remove-member="${m.id}" data-remove-action="${m.banned ? 'restore' : 'remove'}">${m.banned ? 'Restore Access' : 'Remove Member'}</button>` : ''}
    </div>
  `).join('');
  ...
  listEl.querySelectorAll('[data-remove-member]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const action = btn.dataset.removeAction;
      const confirmMsg = action === 'remove'
        ? "Remove this member's access? They won't be able to log in anymore, but their past posts and history stay intact. Reversible any time."
        : "Restore this member's access? They'll be able to log in again.";
      if (!confirm(confirmMsg)) return;

      btn.disabled = true;
      const { error } = await supabaseClient.functions.invoke('manage-member', { body: { targetUserId, action } });
      if (error) { alert('Could not update member: ' + error.message); btn.disabled = false; return; }
      loadAdminPanel();
    });
  });
}

let expandedAdminDays = new Set();
let currentAdminCheckinData = null;

async function loadCheckinsAdminList() {
  if (!currentProfile || !currentProfile.is_admin) return;
  const listEl = document.getElementById('checkins-admin-member-list');
  listEl.innerHTML = skeletonHtml();

  const { data: members } = await supabaseClient
    .from('profiles')
    .select('id, name, avatar_url, has_clients')
    .order('name', { ascending: true });

  adminMembersCache = members || [];

  if (!adminMembersCache.length) {
    listEl.innerHTML = `<div class="empty-state">No members yet.</div>`;
    return;
  }

  listEl.innerHTML = adminMembersCache.map(m => `
    <button class="checkins-admin-row" data-view-checkins="${m.id}">
      ${avatarHtml(m, 'small')}
      <span class="checkins-admin-row-name">${escapeHtml(m.name)}${m.id === currentUser.id ? ' (you)' : ''}</span>
      <span class="checkins-admin-row-arrow">→</span>
    </button>
  `).join('');

  listEl.querySelectorAll('[data-view-checkins]').forEach(btn => {
    btn.addEventListener('click', () => openAdminCheckinModal(btn.dataset.viewCheckins));
  });
}

async function openAdminCheckinModal(memberId) {
  const member = adminMembersCache.find(m => m.id === memberId);
  if (!member) return;

  expandedAdminDays = new Set();
  document.getElementById('admin-checkin-avatar').innerHTML = avatarInnerHtml(member);
  document.getElementById('admin-checkin-name').textContent = member.name;
  document.getElementById('admin-checkin-body').innerHTML = skeletonHtml();
  document.getElementById('admin-checkin-overlay').classList.remove('hidden');
  ...
  currentAdminCheckinData = { member, rows: rows || [] };
  renderAdminCheckinModal();
}

function renderAdminCheckinModal() {
  const { member, rows } = currentAdminCheckinData;
  const { streak, currentLevel } = computeStreakAndLevel(rows);
  const recentRows = rows.slice(0, 7);

  const sumCounter = (itemId) => recentRows.reduce((sum, r) => sum + (Number(r.items && r.items[itemId]) || 0), 0);
  const coldDmsTotal = sumCounter('cold_dms');
  const salesCallsTotal = sumCounter('sales_calls');

  const daysHtml = recentRows.map(row => {
    const expanded = expandedAdminDays.has(row.checkin_date);
    const visible = adminVisibleItemsForSnapshot(row.items, member.has_clients);
    const doneCount = visible.filter(item => isSnapshotItemComplete(row.items, item)).length;

    const detailHtml = visible.map(item => {
      const itemsSnap = row.items || {};
      if (item.type === 'counter') {
        const val = Number(itemsSnap[item.id]) || 0;
        const done = val >= item.target;
        return `<div class="admin-day-item ${done ? 'done' : ''}"><span>${done ? '✓' : '○'} ${escapeHtml(item.label)}</span><span class="admin-day-item-val">${val} / ${item.target}</span></div>`;
      }
      const done = !!itemsSnap[item.id];
      return `<div class="admin-day-item ${done ? 'done' : ''}"><span>${done ? '✓' : '○'} ${escapeHtml(item.label)}</span></div>`;
    }).join('');

    return `
      <div class="admin-checkin-day ${row.completed ? 'complete' : ''}">
        <button class="admin-checkin-day-header" data-toggle-day="${row.checkin_date}">
          <span>${expanded ? '▾' : '▸'} ${row.checkin_date}</span>
          <span class="admin-checkin-day-status">${row.completed ? '✓ Complete' : `${doneCount}/${visible.length}`}</span>
        </button>
        <div class="admin-checkin-day-detail ${expanded ? '' : 'hidden'}">${detailHtml}</div>
      </div>
    `;
  }).join('') || '<div class="empty-state">No check-ins logged yet.</div>';

  const journalDays = recentRows.filter(row => row.items && (row.items.journal_win || row.items.journal_improve || row.items.journal_free));
  const journalHtml = journalDays.length ? journalDays.map(row => `
    <div class="admin-journal-entry">
      <div class="admin-journal-date">${row.checkin_date}</div>
      ${row.items.journal_win ? `<div><strong>Win:</strong> ${escapeHtml(row.items.journal_win)}</div>` : ''}
      ${row.items.journal_improve ? `<div><strong>Would change:</strong> ${escapeHtml(row.items.journal_improve)}</div>` : ''}
      ${row.items.journal_free ? `<div><strong>Free write:</strong> ${escapeHtml(row.items.journal_free)}</div>` : ''}
    </div>
  `).join('') : '<div class="empty-state">No journal entries in the last week.</div>';

  document.getElementById('admin-checkin-body').innerHTML = `
    <div class="admin-checkin-stats">
      <div><span class="admin-checkin-stat-num">${streak}</span><span class="admin-checkin-stat-label">Day Streak</span></div>
      <div><span class="admin-checkin-stat-num">${escapeHtml(currentLevel.name)}</span><span class="admin-checkin-stat-label">Level</span></div>
      <div><span class="admin-checkin-stat-num">${coldDmsTotal}</span><span class="admin-checkin-stat-label">Cold DMs (7d)</span></div>
      <div><span class="admin-checkin-stat-num">${salesCallsTotal}</span><span class="admin-checkin-stat-label">Sales Calls (7d)</span></div>
    </div>
    <div class="admin-checkin-section-title">Last 7 Days <span class="admin-checkin-hint">tap a day for full detail</span></div>
    ${daysHtml}
    <div class="admin-checkin-section-title">Recent Journal Entries</div>
    ${journalHtml}
  `;
  ...
  document.querySelectorAll('[data-toggle-day]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (expandedAdminDays.has(date)) expandedAdminDays.delete(date); else expandedAdminDays.add(date);
      renderAdminCheckinModal();
    });
  });
}

function adminVisibleItemsForSnapshot(itemsSnapshot, hasClients) {
  const snap = itemsSnapshot || {};
  return CHECKLIST_ITEMS.filter(item => {
    if (item.clientsOnly && !hasClients) return false;
    if (item.skipIfChecked && snap[item.skipIfChecked]) return false;
    return true;
  });
}

function isSnapshotItemComplete(itemsSnapshot, item) {
  const snap = itemsSnapshot || {};
  const val = snap[item.id];
  return item.type === 'counter' ? (Number(val) || 0) >= item.target : !!val;
}
...
function initProfileViewer() {
  document.body.addEventListener('click', (e) => {
    ...
    if (btn) {
      e.stopPropagation();
      showProfileModal(btn.dataset.viewProfile);
    }
  });
  ...
  document.getElementById('admin-checkin-close').addEventListener('click', () => {
    document.getElementById('admin-checkin-overlay').classList.add('hidden');
  });
  document.getElementById('admin-checkin-overlay').addEventListener('click', (e) => {
    if (e.target.id === 'admin-checkin-overlay') e.target.classList.add('hidden');
  });
}
...
async function showProfileModal(userId) {
  ...
  setAvatarInto('view-profile-avatar', profile);
...
// ================= NOTIFICATION BELL =================
const NOTIF_LABELS = {
  mention: (n) => `mentioned you in ${n.content_where || 'a post'}`,
  comment: () => 'commented on your post',
  reaction: (n) => `reacted ${n.content || ''} to your post`,
  dm: () => 'sent you a message',
  announcement: () => 'posted a new announcement',
};
...
async function loadNotifications() {
  const { data } = await supabaseClient
    .from('notifications')
    .select('id, type, actor_id, content, link_view, link_id, read, created_at, profiles!notifications_actor_id_fkey(name, avatar_url)')
    .eq('user_id', currentUser.id)
    .order('created_at', { ascending: false })
    .limit(30);
...
function renderNotifBell() {
  ...
  if (!notifCache.length) {
    listEl.innerHTML = `<div class="empty-state">Nothing yet.</div>`;
    return;
  }

  listEl.innerHTML = notifCache.map(n => {
    const actorName = n.profiles ? n.profiles.name : 'Someone';
    const verb = NOTIF_LABELS[n.type] ? NOTIF_LABELS[n.type](n) : 'did something';
    return `
      <div class="notif-item ${n.read ? '' : 'unread'}" data-notif-id="${n.id}" data-link-view="${n.link_view || ''}" data-link-id="${n.link_id || ''}">
        ${avatarHtml(n.profiles, 'small')}
        <div class="notif-item-body">
          <div class="notif-item-text"><strong>${escapeHtml(actorName)}</strong> ${escapeHtml(verb)}</div>
          ${n.content && n.type !== 'reaction' ? `<div class="notif-item-snippet">${escapeHtml(truncate(n.content, 80))}</div>` : ''}
...
async function handleNotifClick(el) {
  ...
  if (!linkView) return;
  switchView(linkView);
  if (linkView === 'messages' && linkId) {
    setTimeout(() => openDmThread(linkId), 50);
  }
}

// ================= WELCOME TUTORIAL =================
function initTutorial() {
  document.getElementById('tutorial-close-btn').addEventListener('click', async () => {
    document.getElementById('tutorial-overlay').classList.add('hidden');
    currentProfile.tutorial_seen = true;
    await supabaseClient.from('profiles').update({ tutorial_seen: true }).eq('id', currentUser.id);
  });
...
function maybeShowTutorial() {
  if (currentProfile && !currentProfile.tutorial_seen) {
    document.getElementById('tutorial-overlay').classList.remove('hidden');
  }
}
...
function initPostSearch() {
  ['wins', 'board', 'announcements', 'introductions'].forEach(channel => {
    const input = document.getElementById(`search-${channel}`);
    if (input) input.addEventListener('input', () => applyChannelFilter(channel));
  });
}
...
function avatarHtml(profile, sizeClass = '') {
  ...
  const initial = escapeHtml(name.trim().charAt(0).toUpperCase() || '?');
  if (profile && profile.avatar_url) {
    return `<div class="avatar-circle ${sizeClass}" style="background:transparent;"><img src="${escapeHtml(profile.avatar_url)}" alt="${initial}" /></div>`;
  }
