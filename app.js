...
function initComposers() {
  ['wins', 'board', 'announcements'].forEach(channel => {
    ...
    postBtn.addEventListener('click', async () => {
      ...
      const { error } = await supabase.from('posts').insert({ user_id: currentUser.id, channel, content });
...
async function loadChannel(channel) {
  ...
  const [{ data: reactions }, { data: comments }] = await Promise.all([
    ...
    supabase.from('comments').select('id, post_id, user_id, content, created_at, profiles(name)').in('post_id', postIds).order('created_at', { ascending: true }),
  ]);
...
async function toggleReaction(postId, emoji, channel) {
  ...
  if (existing) {
    ...
    await supabase.from('reactions').insert({ post_id: postId, user_id: currentUser.id, emoji });
  }
  loadChannel(channel);
}
...
function initCheckin() {
  ...
  clientsToggle.addEventListener('change', async () => {
    ...
    await supabase.from('profiles').update({ has_clients: clientsToggle.checked }).eq('id', currentUser.id);
