  ...
  ['wins', 'board', 'announcements', 'introductions'].forEach(channel => {
    ...
    postBtn.addEventListener('click', async () => {
      ...
      const { error } = await supabaseClient.from('posts').insert({
        user_id: currentUser.id, channel, content, title: title || null,
      });
...
async function loadChannel(channel) {
  ...
  const { data: posts, error } = await supabaseClient
    .from('posts')
    .select('id, title, content, created_at, user_id, profiles(name, avatar_url)')
    .eq('channel', channel)
...
function initIntroGate() {
  ...
  submitBtn.addEventListener('click', async () => {
    ...
    const { error } = await supabaseClient.from('posts').insert({
      user_id: currentUser.id, channel: 'introductions', content, title: title || null,
    });
...
async function loadIntroGateFeed() {
  const { data: posts } = await supabaseClient
    .from('posts')
    .select('id, title, content, created_at, profiles(name, avatar_url)')
    .eq('channel', 'introductions')
    .order('created_at', { ascending: false })
    .limit(30);
